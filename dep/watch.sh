...
#watch "kubectl -n=demo-dep get pods" "2"
desc "Watch pods updates..."
run "kubectl -n=demo-dep get pods -l demo=deployment -L version -w"

#kubectl exec curl -- curl -sI http://nginx | grep HTTP


#target="$1"

#while true; do
  #kubectl --namespace=demo-dep get rs -l demo=deployment \
      #-o go-template='{{range .items}}{{.metadata.name}} {{.metadata.labels}}{{"\n"}}{{end}}' \
      #| while read NAME LABELS; do
    #if echo "$LABELS" | grep -q "$target"; then
      #trap "exit" INT
      #while true; do
        #kubectl --namespace=demo-dep get rs "$NAME" \
